...
const init = async () => {
  try {
    ...
    log.info(`Run ${chalk.cyan(packageManager === 'yarn' ? 'yarn start' : 'npm start')} to start your new site in your browser.`);
    log.info(`Run ${chalk.cyan(packageManager === 'yarn' ? 'yarn build' : 'npm run build')} to build it into the destination folder.`);
  } catch (error) {
